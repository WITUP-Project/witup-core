...
/**
 * Builds SootUp graphs that are required to build a Code Property Graph. It is effectively the same
 * implementation as SootUp's constructor for the CPG, but we control the order of graph creation to
 * avoid recomputing them. We also discard the AST nodes at this stage. Later stages of the pipeline
 * need separate graphs during different parts of the analysis, so it's better to construct them
 * separately than to split them out later on.
 */
public final class SootUpGraphBuilder {
  public SootUpPropertyGraphs buildPropertyGraphs(final JavaSootMethod m) {
    ...
    return new SootUpPropertyGraphs(m.getSignature().toString(), cfg, cdg, ddg, cpg);
  }
}
